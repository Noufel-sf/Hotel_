...
const destinations = (orders: Order[]) => [...new Set(orders.map((o) => o.destination))].sort();
...
export default function BookingRecords() {
  ...
  const [sortField, setSortField] = useState<'createdAt' | 'customer' | 'totalPrice'>('createdAt');
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      const av = getValue(a);
      const bv = getValue(b);
      if (sortField === 'totalPrice') return sortDir === 'asc' ? Number(av) - Number(bv) : Number(bv) - Number(av);
  ...
  const toggleSort = (field: 'createdAt' | 'customer' | 'totalPrice') => {
    if (sortField === field) {
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ink-900/5 text-left">
                  <Th>Order ID</Th>
                  <SortableTh label="Customer" active={sortField === 'customer'} onClick={() => toggleSort('customer')} />
                  <Th>Passport</Th>
                  <Th>Phone</Th>
                  <Th>Hotel</Th>
                  <Th>Destination</Th>
                  <Th>Dates</Th>
                  <Th>Rooms</Th>
                  <SortableTh label="Total" active={sortField === 'totalPrice'} onClick={() => toggleSort('totalPrice')} />
                  <SortableTh label="Created" active={sortField === 'createdAt'} onClick={() => toggleSort('createdAt')} />
                  <Th>Payment</Th>
                </tr>
              </thead>
              <tbody>
